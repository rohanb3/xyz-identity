...
namespace Xyzies.SSO.Identity.Tests
{
    public class UsersControllerTest : IClassFixture<TestServerInitializer>
    {
        ...
        [InlineData(Consts.Roles.SupportAdmin)]
        ...
        [InlineData(Consts.Roles.SupportAdmin, "xyzies.authorization.reviews.admin")]
        ...
        [InlineData(new string[] { Consts.Roles.OperationsAdmin, Consts.Roles.SalesRep, Consts.Roles.Operator }, Consts.Roles.SystemAdmin)]
        [InlineData(new string[] { Consts.Roles.OperationsAdmin, Consts.Roles.SuperAdmin }, Consts.Roles.SupportAdmin)]
        ...
        public async Task ShouldReturnUsersByCities()
        {
            ...
            //Assert.True(users.Result.All(x => usersCities.Contains(x.City)));
            //Assert.DoesNotContain(users.Result, x => !usersCities.Contains(x.City));
            //Assert.Equal(usersCities.Count(), users.Result.Select(x => x.City).Distinct().Count());
        }
